...
from api_test.common.common import set_step_id_map
from api_test.models import AutomationCaseApi, AutomationTaskRunTime, GlobalHost, Project,AutomationTestTask, AutomationTestResult
...
def automation_task(host_id, project_id, case_id, taskName):
    ...
    for j in case:
        data = AutomationCaseApi.objects.filter(automationTestCase=j).order_by('id')
        # Build step→api_id mapping for this case group (step 1 = first API)
        step_id_map = {idx + 1: api.pk for idx, api in enumerate(data)}
        set_step_id_map(step_id_map)
        # Clear old test results to prevent stale $N.field resolution
        for api in data:
            AutomationTestResult.objects.filter(automationCaseApi=api.pk).delete()
        for i in data:
