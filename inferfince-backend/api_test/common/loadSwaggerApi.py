...
def add_swagger_api(data, user):
    ...
    try:
        ...
        try:
            ...
            # 分组不存在（或 id=0），自动使用/创建默认分组
            obi, _ = ApiGroupLevelFirst.objects.get_or_create(
                name="默认分组", project=obj,
                defaults={"name": "默认分组", "project": obj},
            )
            data["apiGroupLevelFirst_id"] = obi.id
        try:
