...
def creatTask(host_id, _type, start_time, case_id,end_time, project,taskName ,frequency=None, unit=None):
    """
        ...
        :return:
        """
    job_id = _make_job_id(taskName, start_time)
...
def _make_job_id(taskName: str, dt) -> str:
    """Build a scheduler job ID from task name and datetime."""
    if isinstance(dt, str):
        dt = time.mktime(time.strptime(dt[:19], "%Y-%m-%d %H:%M:%S"))
    elif hasattr(dt, 'timetuple'):
        dt = time.mktime(dt.timetuple())
    return taskName + str(dt).split('.')[0]


def delTask(start_time, taskName):
    """Delete a scheduled job by task name and start time."""
    job_id = _make_job_id(taskName, start_time)
    try:
