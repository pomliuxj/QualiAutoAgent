...
# Per-request project_id overrides the env var (set by chat_controller)
_runtime_project_id = contextvars.ContextVar(
    "project_id"
)


def set_project_id(project_id: int) -> None:
    """Set the project_id for the current request context."""
    _runtime_project_id.set(project_id)


def get_project_id() -> int:
    """Get the current request's project_id, falling back to env var."""
...
def get_runtime_case_group_id() -> int:
    if _runtime_case_group_id:
        return _runtime_case_group_id
    return get_default_case_id()


def get_default_case_id() -> int:
    """Get the first automation case group for the current project, or create one."""
    pid = get_project_id()
    url = f"{AUTOMATION_BASE_URL}/api/automation/case_list?project_id={pid}"
    try:
        ...
        pass
    # No case group found — create a default one
    return _create_default_case_group(pid)


def _create_default_case_group(project_id: int) -> int:
    """Create a default automation case group and return its id."""
    # First ensure a group exists
    list_url = f"{AUTOMATION_BASE_URL}/api/automation/group?project_id={project_id}"
    group_id = None
    try:
        resp = requests.get(list_url, headers=_get_auth_headers(), timeout=10)
    ...
    if not group_id:
        try:
            ...
            payload = {"project_id": project_id, "name": "AI自动生成"}
            resp = requests.post(create_url, json=payload, headers=_get_auth_headers(), timeout=10)
            group_id = resp.json().get("data", {}).get("group_id", 1)
        except Exception:
            group_id = 1

    # Now create the case group
    create_url = f"{AUTOMATION_BASE_URL}/api/automation/add_case"
    payload = {"project_id": project_id, "caseName": "AI生成-默认", "automationGroupLevelFirst_id": group_id}
    try:
        resp = requests.post(create_url, json=payload, headers=_get_auth_headers(), timeout=10)
        case_id = resp.json().get("data", {}).get("case_id", 1)
        return case_id
    except Exception:
        return 1


def get_default_host_id() -> int:
    """Get the first host for the current project, or create one."""
    pid = get_project_id()
    url = f"{AUTOMATION_BASE_URL}/api/global/host_total?project_id={pid}"
    try:
        resp = requests.get(url, headers=_get_auth_headers(), timeout=10)
        ...
        pass
    # No host — create a default one
    create_url = f"{AUTOMATION_BASE_URL}/api/global/host_total"
    payload = {
        "project_id": pid,
...
class AddApiTool(BaseTool):
    ...
    def _find_existing_case(self, case_name: str, headers: Dict[str, str]):
        ...
        url = (
            ...
            f"?project_id={get_project_id()}&name={case_name}"
        )
        try:
    ...
    def _ensure_group(self, headers: Dict[str, str]) -> int:
        ...
        with lock:
            ...
            list_url = f"{AUTOMATION_BASE_URL}/api/automation/group?project_id={get_project_id()}"
            try:
            ...
            create_url = f"{AUTOMATION_BASE_URL}/api/automation/add_group"
            payload = {"project_id": get_project_id(), "name": group_name}
            try:
...
class SearchApiInfoTool(BaseTool):
    ...
    def _run(self, case_id: str) -> Dict[str, Any]:
        ...
        url = (
            ...
            f"?project_id={get_project_id()}&case_id={get_runtime_case_group_id()}&api_id={case_id}"
        )
        try:
...
class ApiDocInfoTool(BaseTool):
    ...
    def _run(self, api_id: str) -> Dict[str, Any]:
        ...
        url = (
            ...
            f"?project_id={get_project_id()}&api_id={api_id}"
        )
        try:
            response = requests.get(url, headers=_get_auth_headers())
            response.raise_for_status()
...
class ExecuteScenarioStepTool(BaseTool):
    ...
    def _run(self, case_id: str, host_id: str = "", project_id: str = "") -> Dict[str, Any]:
        ...
        url = f"{AUTOMATION_BASE_URL}/api/automation/start_test_sequential"
        payload = {
            "project_id": int(project_id) if project_id else get_project_id(),
            "case_id": get_runtime_case_group_id(),
            "host_id": int(host_id) if host_id else get_default_host_id(),
            "id": [int(case_id)],
        }
        logger.info(
