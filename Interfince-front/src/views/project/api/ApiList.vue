  ...
  <div class="list-page">
    <div class="toolbar-card">
      <div class="tb-row">
        ...
        <div class="tb-right">
          <el-button size="small" @click="DownloadApi"><i class="fa fa-download"></i> 下载文档</el-button>
          <el-button size="small" @click="loadSwaggerApi = true; getApiGroup()"><i class="fa fa-upload"></i> 导入Swagger</el-button>
        </div>
      </div>
    </div>
    ...
    <el-dialog title="导入 Swagger 接口" :visible.sync="loadSwaggerApi" :close-on-click-modal="false" width="520px">
      <el-input v-model.trim="swaggerUrl" placeholder="输入 Swagger 接口地址" style="margin-bottom:14px" />
      <p v-if="!swaggerUrl" style="color:#dc2626;font-size:12px;margin:0 0 12px">地址不能为空</p>
      ...
      <div slot="footer"><el-button @click="loadSwaggerApi = false">取消</el-button><el-button type="primary" @click="addSubmit" :loading="addLoading">导入</el-button></div>
    </el-dialog>
...
export default {
  data() {
    return {
      ...
      loadSwaggerApi: false, addLoading: false, swaggerUrl: '',
    }
  },
  methods: {
    checkMockStatus(row) {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.code === '999999') { self.$message.success({ message: data.msg, center: true }); self.getApiList() }
    ...
    getApiList() {
      ...
      $.ajax({
        type: 'get', url: test + '/api/api/api_list', async: true,
    ...
    DownloadApi() {
      $.ajax({
        type: 'get', url: test + '/api/api/Download', async: true, data: { project_id: this.$route.params.project_id },
        headers: { Authorization: 'Token ' + JSON.parse(localStorage.getItem('token')) }, timeout: 5000,
        success: function (data) { if (data.code === '999999') { window.open(test + '/api/api/download_doc?url=' + data.data) } },
      })
    },
    ...
    addSubmit() {
      let self = this; this.addLoading = true
      if (!this.swaggerUrl) { this.addLoading = false; return }
      $.ajax({
        type: 'post', url: test + '/api/api/lead_swagger', async: true,
        data: JSON.stringify({ project_id: Number(this.$route.params.project_id), url: this.swaggerUrl, apiGroupLevelFirst_id: Number(self.updateGroupForm.firstGroup) }),
        headers: { 'Content-Type': 'application/json', Authorization: 'Token ' + JSON.parse(localStorage.getItem('token')) },
        success: function (data) { self.addLoading = false; if (data.code === '999999') { self.$message({ message: '添加成功', center: true, type: 'success' }); self.loadSwaggerApi = false; self.getApiList() } else { self.$message.error({ message: '导入失败', center: true }) } },
      })
    },
  },
...
.toolbar-card {
  background: $surface; border: 1px solid $border; border-radius: 10px; padding: 14px 18px; margin-bottom: 14px; box-shadow: $shadow-sm;
...
.data-table {
  :deep(th) { background: #f9fafb; color: $text-secondary; font-size: 12px; font-weight: 600; }
  :deep(.el-table__empty-text) { color: $text-tertiary; }
}

.table-link { color: $accent; text-decoration: none; font-weight: 500; &:hover { text-decoration: underline; } }

.method-tag {
  display: inline-block; font-size: 11px; font-weight: 700; letter-spacing: 0.03em; padding: 2px 8px; border-radius: 3px; color: #fff;
  &.method--get { background: #059669; } &.method--post { background: #1d4ed8; } &.method--put { background: #d97706; } &.method--delete { background: #dc2626; }
}

.mock-badge { display: inline-block; font-size: 10px; font-weight: 700; letter-spacing: 0.05em; padding: 3px 9px; border-radius: 3px; cursor: pointer; user-select: none;
  &.mock--on { background: #ecfdf5; color: #059669; } &.mock--off { background: #f3f4f6; color: #9ca3af; }
}
